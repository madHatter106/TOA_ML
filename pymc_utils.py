class PyMCModel:
    ...
    def plot_model_fits2(self, y_obs, y_pred=None, title=None, ax=None, ci=0.95):
        if y_pred is None:
            y_pred = self.trace_.get_values('mu')
        y_pred_mean = np.mean(y_pred, axis=0)
        y_pred_hpd = pm.hpd(y_pred, alpha=1-ci)
        y_obs = y_obs.values
        ...
        ax.plot(xi, y.values[iy], marker='.', ls='',
                ...
                label='observed')
        ax.plot(xi, ypred_mean[iy], marker='o', color='indigo',
                ls='', markeredgecolor='k', alpha=0.5, label='predicted avg.')
        ax.fill_between(xi, hpd[iy, 0], hpd[iy, 1], color='k', alpha=0.5,
